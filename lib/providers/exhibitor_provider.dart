...
class ExhibitorProvider with ChangeNotifier {
  ...
  ExhibitorModel findById(String exhibitorId, String eventId) {
    return _exhibitors.firstWhere(
      (exhibitor) =>
          exhibitor.id == exhibitorId && exhibitor.eventId == eventId,
    );
  }

  List<ExhibitorModel> favoriteExhibitors(String eventId) {
    return _exhibitors.where((exhibitor) => exhibitor.eventId == eventId && exhibitor.isFavorite == true).toList();
  }
}
